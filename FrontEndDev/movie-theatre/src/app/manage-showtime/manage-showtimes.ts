...
export class ManageShowtimesComponent implements OnInit {
    ...
    loadMovies(): void {
        this.http.get<Movie[]>('http://localhost:8080/api/movies').subscribe({
            next: (data) => {
                this.movies = data;
    ...
    loadShowtimes(): void {
        this.http.get<Showtime[]>('http://localhost:8080/api/showtimes').subscribe({
            next: (data) => {
                this.showtimes = data;
                this.cdr.detectChanges();
            },
    ...
    addShowtime(): void {
        ...
        this.http.post<Showtime>('http://localhost:8080/api/showtimes', this.showtimeForm).subscribe({
            next: () => {
                this.ngZone.run(() => {
                    this.showtimeError = '';
