...
public class BookingService {
    ...
    /**
     * Simulated decline CVVs — mirrors how Stripe test mode uses magic values.
     *
     * Use these CVVs to demonstrate payment failure during a demo:
     * 000 → Generic decline (card declined)
     * 0000 → Generic decline (card declined, 4-digit variant)
     * 999 → Insufficient funds
     * 9999 → Insufficient funds (4-digit variant)
     * 123 → Always succeeds (can use in demo to show happy path)
     *
     * Any other valid 3-4 digit CVV also succeeds (normal simulation behavior).
     */
    ...
    public BookingResponse createBooking(BookingRequest request) {
        ...
        // Simulated payment authorization — runs BEFORE saving anything to DB
    ...
    /**
     * Simulated payment authorization gate.
     * Checks CVV against known test-decline values before any DB write occurs.
     * This mirrors the behaviour of a real payment gateway returning a decline
     * code.
     */
    ...
    private void sendBookingConfirmation(BookingHistory b) {
        String html = """
                ...
                b.getSeatNumbers(), b.getTotalAmount().setScale(2, RoundingMode.HALF_UP),
